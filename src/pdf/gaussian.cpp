...
namespace BFL
{
  ...
  */
  ...
  Probability Gaussian::ProbabilityGet(const ColumnVector& input) const
  {
    ...
    _diff = input; // x
    _diff -= _Mu;  // x-u
    _Sigma_inverse.multiply(_diff,_tempColumn); //TODO 感觉是  _tempColumn = _Sigma_inverse *  _diff
  ...
  {
    std::cout << " Gaussian::ExpectedValueGet " << std::endl;
    return _Mu;
  }
