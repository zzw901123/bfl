...
namespace BFL
{
  ...
  */
  LinearAnalyticConditionalGaussian::LinearAnalyticConditionalGaussian(const vector<Matrix> & ratio,
    ...
    , _ratio(ratio)
    , _mean_temp(DimensionGet())  //继承pdf DimensionGet()为参数维度默认为0
    , _arg(DimensionGet())
  {
  ...
  {
    std::cout << "LinearAnalyticConditionalGaussian::ExpectedValueGet()" << std::endl;
    _mean_temp = 0.0;
    for (unsigned int i=0; i < NumConditionalArgumentsGet() ; i++)
    {
  ...
  {
    std::cout << "LinearAnalyticConditionalGaussian::dfGet" << std::endl;
    assert(i < NumConditionalArgumentsGet());
    return _ratio[i]; //TODO 控制参数
  }
