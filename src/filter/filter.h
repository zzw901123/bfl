...
namespace BFL
{
  ...
  */
  template <typename StateVar, typename MeasVar> class Filter
  {
    ...
    /// Pointer to the Posterior Pdf. 后验概率分布
    ...
    */
    ...
    /// Represents the current timestep of the filter 当前过滤器的时间步长
    /** @todo Check wether this really belongs here
     */
    int _timestep;

    /// Actual implementation of Update, varies along filters
    /** @param sysmodel pointer to the used system model 指向所用系统模型的指针
	  @param u input param for proposal density 提议密度 输入参数
	  @param measmodel pointer to the used measurementmodel 指向使用的测量模型的指针
	  @param z measurement param for proposal density 提议密度 测量参数
	  @param s sensor param for proposal density 提议密度 传感器参数
    */
