...
protected:
  ...
  */
  ...
  /** Calculate Kalman filter System Update  计算卡尔曼滤波器系统更新 预测更新
      \f[ x_k = J \f]
      \f[ P_k = F.P_{k-}.F' + Q \f]
  */
  ...
  /** Calculate Kalman filter Measurement Update 计算卡尔曼滤波器测量更新 测量更新
      \f[ x_k = x_{k-} + K.(z - Z) \f]
      \f[ P_k = (I-K.H).P_{k-} \f]
      with
  ...
  */
  ...
  /// System Update
  /** Update the filter's Posterior density using the deterministic 使用确定性更新过滤器的后验密度  系统和系统模型的输入
      inputs to the system and the system model
      @param sysmodel pointer to the system model the filter should use 指向过滤器应使用的系统模型的指针
      @param u input to the system 系统输入
  */
