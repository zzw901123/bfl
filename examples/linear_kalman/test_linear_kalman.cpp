...
#include "../mobile_robot_wall_cts.h" // 读取的一部分参数（比如机器人的初始位置估计）
...
int main(int argc, char** argv)
{
  ...
  SymmetricMatrix sysNoise_Cov(2); // 通过协方差的数值大小 判断这两个变量同向或反向的程度
  sysNoise_Cov = 0.0;
  ...
  Gaussian system_Uncertainty(sysNoise_Mu, sysNoise_Cov); // 均值与协方差矩阵一起定义二维高斯分布

  // create the model
  LinearAnalyticConditionalGaussian sys_pdf(AB, system_Uncertainty); // 线性条件概率密度函数（pdf）
  LinearAnalyticSystemModelGaussianUncertainty sys_model(&sys_pdf); // 系统的概率密度函数创造出整个预测系统的模型
  ...
  // create the model
  LinearAnalyticConditionalGaussian meas_pdf(H, measurement_Uncertainty); //以创建测量可能的概率密度模型，这个概率密度模型由表示测量系统本身的矩阵H以及代表测量不确定性的高斯分布构成
  ...
  Gaussian prior(prior_Mu,prior_Cov); //代表初始估计与不确定度的先验分布
   ...
   ******************************/
  ExtendedKalmanFilter filter(&prior); // 构造滤波器
